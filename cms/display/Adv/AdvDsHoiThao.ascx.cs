...
public partial class cms_display_Adv_AdvDsHoiThao : System.Web.UI.UserControl
{
    ...
    private string GetListAdv(string igid, string cssImage)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += @"
<div class='list-news__item fade-up'>
        <div class='list-news__wrap'>
            <div class='img'>
                <div class='img__crop'>
                   <img  alt='" + dt.Rows[i][ItemsColumns.ViTitle] + @"' src='" + UrlExtension.WebisteUrl + pic + "/" + dt.Rows[i][ItemsColumns.ViImage] + @"' />
                </div>
            </div>
            <h4 class='list-news__ttl'><a href=''> " + dt.Rows[i][ItemsColumns.ViTitle] + @"</a></h4>
            <p class='txtBase'>
               " + dt.Rows[i][ItemsColumns.ViSeoTitle] + @"
            </p>
