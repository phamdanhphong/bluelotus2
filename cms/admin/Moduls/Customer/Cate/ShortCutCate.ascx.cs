...
public partial class cms_admin_Moduls_Customer_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Customer, TypePage.Cate, DdlGroupCustomer.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupCustomer.Items.Clear();
        DdlGroupCustomer.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupCustomer.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupCustomer.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.CustomerKeyword.CapNhatDanhMuc;
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_Customer", false, false, "", false);
            ...
            ddlLoaiBaiViet.SelectedValue = dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.VGSEOMETAPARAMSColumn].ToString();
        ...
        {
            LtInsertUpdate.Text = Developer.CustomerKeyword.TaoDanhMuc;
            btn_insert_update.Text = "Đồng ý";
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupCustomer.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", ddlLoaiBaiViet.SelectedValue, vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        ...
        else
        {
            ...
            if (DdlGroupCustomer.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupCustomer.SelectedValue);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupCustomer.SelectedValue;
        txtDesc.Text = "";
