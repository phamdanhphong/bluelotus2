...
public partial class cms_admin_Moduls_Customer_Item_ControlItem : System.Web.UI.UserControl
{
    protected string app = CodeApplications.Customer;
    protected string pic = FolderPic.Customer;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string sortCookiesName = SortKey.SortCustomerItems;
    private string p = "1";
    ...
    protected string LinkUpdate(string iid)
    {
        ...
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Customer, TypePage.UpdateItem, iid, NumberShowItem, p);
        }
        else
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Customer, TypePage.UpdateItem, iid);
        }
    }
    ...
    protected string LinkCreate()
    {
        ...
        return LinkAdmin.GoAdminCategory(CodeApplications.Customer, TypePage.CreateItem, igidUpdate);
    }

    void GetCustomer(string order)
    {
        if (!igid.Equals(""))
        {
        ...
        LtPagging.Text = PagingExtension.SpilitPages(Convert.ToInt32(dt.Rows[0]["TotalRows"]),
                                                      ...
                                                      LinkAdmin.UrlAdmin(CodeApplications.Customer, TypePage.Item,
    ...
    void GetGroupsInDdl()
    {
        ...
        ddlCateSearch.Items.Add(new ListItem(Developer.CustomerKeyword.TatCaDanhMuc, ""));
    ...
    {
        string key = tbKeySearch.Text + "&name=" + tbTitleSearch.Text;
        Response.Redirect(LinkAdmin.GoAdminCategory(CodeApplications.Customer, TypePage.Item, ddlCateSearch.SelectedValue,
