...
public partial class cms_admin_Moduls_Customer_Item_ShortCutItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.Customer;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string pic = FolderPic.Customer;
    private string propertyModul = CodeApplications.CustomerProperty;
    ...
    {       
        //Ẩn hiện chức năng Thuộc tính khách hàng
        if (CustomerConfig.KeyHienThiQuanLyThuocTinhKhachHang) pnThuocTinhKhachHang.Visible = true;
        else pnThuocTinhKhachHang.Visible = false;      
    }
    #endregion
    ...
    void KhoiTaoXuLyAnh()
    {
        ...
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhCustomer_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhCustomer_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhCustomer_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhCustomer_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhCustomer_TrongSuot, language);
        #endregion
    ...
    private string LinkRedrect()
    {
        ...
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Customer + "&igid=" + ddl_group_product.SelectedValue + "&suc="+TypePage.Item+"&ni=" + ni + "&p=" + p;
        }
        else
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Customer + "&igid=" + ddl_group_product.SelectedValue + "&suc=" + TypePage.Item;
        }
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            hdOldCustomer.Value = dt.Rows[0]["VICONTENT"].ToString();
            // them phan link web site
            txtLinkWebsite.Text = dt.Rows[0][ItemsColumns.ViAuthor].ToString();
            ...
            #region ThuocTinhKhachHang-Chi thực hiện khi chức năng Quản lý thuộc tính được hiển thị
            if (CustomerConfig.KeyHienThiQuanLyThuocTinhKhachHang)
            {
                string properties = "";
                condition = TatThanhJsc.Extension.DataExtension.AndConditon(
    ...
    {
        string contentDetail = ContentExtendtions.ProcessStringContent(txt_content.Text, hdOldCustomer.Value, pic);
        #region Image
        string vimg = "";
        string vimg_thumb = "";
        ...
        if (insert)
        {
            GroupsItems.InsertItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, "", txtLinkWebsite.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", "", "0", "0", "", "", timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, status);
        ...
        else
        {
            ...
            GroupsItems.UpdateItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, "", txtLinkWebsite.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", "", "0", "0", "", HdIitotalview.Value, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, status, iid);
