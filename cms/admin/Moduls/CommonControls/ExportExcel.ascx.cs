...
public delegate void ExportToExcelEventHandler();

public partial class cms_admin_CommonControls_ExportExcel : System.Web.UI.UserControl
{
    ...
    private event ExportToExcelEventHandler export;
    public event ExportToExcelEventHandler Export
    {
        add { export += value; }
        remove { export += null; }
    }
    private event ExportToExcelEventHandler exportAll;
    public event ExportToExcelEventHandler ExportAll
    {
        add { exportAll += value; }
        remove { exportAll += null; }
    }
    ...
    {        
        if (export != null)
        {
            ...
            string fileName = "ExportExcel_" + datetime.ToShortDateString() + "_" + datetime.Hour + "h_" + datetime.Minute + "m_" + datetime.Second + "s";
            if (dsSource.Tables[0].Rows.Count > 0)
                ConvertToExcel(dsSource, fileName);
            else
                ScriptManager.RegisterStartupScript(btOK, btOK.GetType(), "alertNull", "alert('Vui lòng tích chọn các mục muốn xuất ra excel!');", true);
        }
    }
    protected void btOKS_Click(object sender, EventArgs e)
    {
        if (exportAll != null)
        {
            exportAll();
            DateTime datetime = DateTime.Now;
            string fileName = "ExportExcels_" + datetime.ToShortDateString() + "_" + datetime.Hour + "_" + datetime.Minute + "_" + datetime.Second;
            ConvertToExcel(dsSource, fileName);
        }
    }
