...
public partial class cms_admin_Moduls_Destination_Cate_ShortCutCate : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    protected string app = CodeApplications.Destination;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        flAnhQuocGia.App = app;
        flAnhQuocGia.Pic = pic;

        flAnhGioiThieu.App = app;
        flAnhGioiThieu.Pic = pic;
        #endregion
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Destination, TypePage.Cate, ddlParentCate.SelectedValue);
    }
    ...
    void LoadParentCate()
    {
        ...
        ddlParentCate.Items.Clear();
        ddlParentCate.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ddlParentCate.Items.Add(new ListItem(DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
    ...
    protected void btOK_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            ...
            string anhQuocGia = flAnhQuocGia.Save(false, contentDetail);
            string anhGioiThieu= flAnhGioiThieu.Save(false, contentDetail);
            string vgparams = StringExtension.GhepChuoi("", anhQuocGia, GoogleMapMarkLocation.ViDo,
                GoogleMapMarkLocation.KinhDo, anhGioiThieu, tbDanSo.Text, tbThuDo.Text, tbDanToc.Text, tbNgonNgu.Text,
                tbTienTe.Text, tbMuiGio.Text, tbMaDTQuocTe.Text, tbThongSoTongQuan.Text);
            ...
            Groups.InsertGroups(language, app, ddlParentCate.SelectedValue, tbTitle.Text, tbDesc.Text, contentDetail,
                ...
                tbSeoDescription.Text, "", "", "", image, vgparams, "", tbOrder.Text, DateTime.Now.ToString(),
        ...
        else
        {
            ...
            string anhQuocGia = flAnhQuocGia.Save(true, contentDetail);
            string anhGioiThieu = flAnhGioiThieu.Save(true, contentDetail);
            string vgparams = StringExtension.GhepChuoi("", anhQuocGia, GoogleMapMarkLocation.ViDo, GoogleMapMarkLocation.KinhDo, anhGioiThieu, tbDanSo.Text, tbThuDo.Text, tbDanToc.Text, tbNgonNgu.Text,
                tbTienTe.Text, tbMuiGio.Text, tbMaDTQuocTe.Text, tbThongSoTongQuan.Text);
            ...
            Groups.UpdateGroups(language, app, tbTitle.Text, tbDesc.Text, contentDetail, tbSeoTitle.Text, tbSeoLink.Text,
                ...
                image, vgparams, "", tbOrder.Text, DateTime.Now.ToString(), DateTime.Now.ToString(),
    ...
    private void SetAnHienCacTruong()
    {
        if (ddlParentCate.SelectedIndex == 0)
        {
            //plOtherInfo.Visible = true;
            flAnhDaiDien.Visible = true;
            flAnhQuocGia.Visible = true;
            flAnhGioiThieu.Visible = true;
        }
        else
        {
            //plOtherInfo.Visible = false;
            flAnhDaiDien.Visible = false;
            flAnhQuocGia.Visible = false;
            flAnhGioiThieu.Visible = true;
        }
    }
}