...
public partial class cms_admin_Product_ShowProductControl_ViewCart : System.Web.UI.Page
{
    private string iid = "";

    private string app =TatThanhJsc.ProductModul.CodeApplications.ProductCart; 
    ...
    {        
        ...
        return NumberExtension.FormatNumber(dongia.ToString(),true,"liên hệ","");
    }
    ...
    protected string LayThongTinKhachHang(string info)
    {
        ...
        if (values["hinhthucthanhtoan"]!=null)
        {
            temp = values["hinhthucthanhtoan"];
            if (temp.Length > 0)
                s += "<b>Hình thức thanh toán:</b> " + temp + "<br/>";
        }
        if (values["noidung"] != null)
        {
            temp = values["noidung"];
            if (temp.Length > 0)
                s += "<b>Nội dung:</b> " + temp + "<br/>";
        }
        #endregion
