...
public partial class cms_admin_Moduls_Product_Item_ControlItem : System.Web.UI.UserControl
{
    protected string app = CodeApplications.Product;
    protected string pic = FolderPic.Product;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string sortCookiesName = SortKey.SortProductItems;
    private string p = "1";
    ...
    protected string LinkUpdate(string iid)
    {
        ...
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Product, TypePage.UpdateItem, iid, NumberShowItem, p);
        }
        else
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Product, TypePage.UpdateItem, iid);
        }
    }
    ...
    protected string LinkCreate()
    {
        ...
        return LinkAdmin.GoAdminCategory(CodeApplications.Product, TypePage.CreateItem, igidUpdate);
    }
    ...
    void GetNews(string order)
    {
        ...
        else
        {
            ...
            if (orderBy.Length < 1)
                orderBy = GroupsColumns.IgorderColumn + "," + ItemsColumns.IiorderColumn + ", DCREATEDATE DESC ";
        }
        ...
        LtPagging.Text = PagingExtension.SpilitPages(Convert.ToInt32(dt.Rows[0]["TotalRows"]),
                                                      ...
                                                      LinkAdmin.UrlAdmin(CodeApplications.Product, TypePage.Item,
    ...
    void GetGroupsInDdl()
    {
        ...
        ddlCateSearch.Items.Add(new ListItem(Developer.ProductKeyword.TatCaDanhMuc, ""));
    ...
    {
        string key = tbKeySearch.Text + "&name=" + tbTitleSearch.Text;
        Response.Redirect(LinkAdmin.GoAdminCategory(CodeApplications.Product, TypePage.Item, ddlCateSearch.SelectedValue,
