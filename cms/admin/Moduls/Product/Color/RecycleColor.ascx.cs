...
public partial class cms_admin_Moduls_Product_Color_RecycleColor : System.Web.UI.UserControl
{
    ...
    void GetAllModul()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                str += "<div class=\"splitc\">|</div>";
                str += "<div class=\"cot7\">";
                str += "<a href=\"javascript:RestoreGroup('" + dt.Rows[i]["IGID"].ToString() + "','" + StatusGroupParent(dt.Rows[i]["IGPARENTID"].ToString()) + "','" + GetSubColor(dt.Rows[i]["IGID"].ToString()) + "','" + dt.Rows[i]["VGNAME"].ToString() + "')\"><span class='iconRestore'><!----></span></a>";
                str += "&nbsp;&nbsp;&nbsp;";
                str += "<a href=\"javascript:DeleteRecGroup('" + dt.Rows[i]["IGID"].ToString() + "','" + GetSubColor(dt.Rows[i]["IGID"].ToString()) + "','" + dt.Rows[i]["VGNAME"].ToString() + "','"+pic+"')\"><span class='iconDelete'><!----></span></a>";
                str += "</div>";
                str += "<div class=\"cbh0\"><!----></div>";
                str += "</div>";
                str += "</div>";
    ...
    string GetSubColor(string igparentid)
    {
        top = "";
        fields = " * ";
        condition = DataExtension.AndConditon(GroupsTSql.GetGroupsByVgapp(Modul),
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (!CountColor(dt.Rows[i]["IGID"].ToString()).Equals("0"))
                {
                    str += GetSubColor(dt.Rows[i]["IGID"].ToString()) + ",";
                }
            }
    ...
    string GetRoadGroup(string igid, string vgColorCurrent)
    {
        string str = "";
        top = "1";
        fields = " * ";
