...
public partial class cms_admin_Moduls_Menu_MenuMain_RecycleMenuMain : System.Web.UI.UserControl
{
    ...
    void GetAllModul()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                str += "&nbsp;&nbsp;&nbsp;";
                str += "<a href=\"javascript:DeleteRecGroup('" + dt.Rows[i]["IGID"].ToString() + "','" + GetSubCate(dt.Rows[i]["IGID"].ToString()) + "','" + dt.Rows[i]["VGNAME"].ToString() + "')\"><span class='iconDelete'><!----></span></a>";
                str += "</div>";
                str += "<div class=\"cbh0\"><!----></div>";
                str += "</div>";
                str += "</div>";
    ...
    string GetRoadGroup(string igid, string vgCateCurrent)
    {
        ...
        top = "1";
        fields = " * ";
        condition = DataExtension.AndConditon(GroupsTSql.GetGroupsByVgapp(app), GroupsTSql.GetGroupsByIgid(igid));
        orderBy = "";
    ...
    string StatusGroupParent(string igparentid)
    {
        ...
        top = " 1 ";
        fields = " * ";
        condition = DataExtension.AndConditon(GroupsTSql.GetGroupsByVgapp(app), GroupsTSql.GetGroupsByIgid(igparentid));
        orderBy = "";
