...
public partial class cms_admin_Moduls_Content_Item_ControlItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.Content;
    protected string pic = FolderPic.Content;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string sortCookiesName = SortKey.SortContentItems;
    private string p = "1";
    ...
    protected string LinkUpdate(string iid)
    {
        ...
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Content, TypePage.UpdateItem, iid, NumberShowItem, p);
        }
        else
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Content, TypePage.UpdateItem, iid);
        }
    }
    ...
    private string LinkCreate()
    {
        ...
        return LinkAdmin.GoAdminCategory(CodeApplications.Content, TypePage.CreateItem, igidUpdate);
    }
    ...
    void GetNews(string order)
    {
        ...
        LtPagging.Text = PagingExtension.SpilitPages(Convert.ToInt32(dt.Rows[0]["TotalRows"]),
                                                      ...
                                                      LinkAdmin.UrlAdmin(CodeApplications.Content, TypePage.Item,
    ...
    void GetGroupsInDdl()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ddl_group_ontab.Items.Add(new ListItem("Tất cả danh mục sản phẩm", ""));
