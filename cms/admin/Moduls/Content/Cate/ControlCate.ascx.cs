...
public partial class cms_admin_Moduls_Content_Cate_ControlCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Content, typePageCreate);        
    }
    ...
    string GetSubCate(string igparentid)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                str += "<div class=\"splitc\">|</div>";
                str += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderGroup_Content('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
                str += "<div class=\"splitc\">|</div>";
                str += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
                str += "<div class=\"splitc\">|</div>";
                str += "<div class=\"cot7\">";
