...
public partial class cms_admin_Moduls_Content_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        #region Đóng dấu ảnh
        if (SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate, language) == "1")
            cbDongDauAnh.Checked = true;
        else
            cbDongDauAnh.Checked = false;
        #region Ảnh làm dấu
        hdLogoImage.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Watermark, language);
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Position, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Horizal, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Vertical, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Rate, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingsExtension.KeyInputImgContentCate_Transparent, language);
        #endregion

        #region Hạn chế kích thước ảnh đại diện
        if (SettingsExtension.GetSettingKey(SettingsExtension.KeyFixSizeImgContentCate, language) == "1")
            cbHanCheKichThuoc.Checked = true;
        else
            cbHanCheKichThuoc.Checked = false;

        tbHanCheW.Text = SettingsExtension.GetSettingKey(SettingsExtension.KeyFixSizeImgContentCate_MaxWidth, language);
        tbHanCheH.Text = SettingsExtension.GetSettingKey(SettingsExtension.KeyFixSizeImgContentCate_MaxHeight, language);
        #endregion

        #region Tạo ảnh nhỏ cho ảnh đại diện
        if (SettingsExtension.GetSettingKey(SettingsExtension.KeyCreateThumbImgContentCate, language) == "1")
            cbTaoAnhNho.Checked = true;
        else
            cbTaoAnhNho.Checked = false;

        tbAnhNhoW.Text = SettingsExtension.GetSettingKey(SettingsExtension.KeyCreateThumbImgContentCate_MaxWidth, language);
        tbAnhNhoH.Text = SettingsExtension.GetSettingKey(SettingsExtension.KeyCreateThumbImgContentCate_MaxHeight, language);
        #endregion
    }

    private string LinkRedrect()
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Content, TypePage.Cate, DdlGroupContent.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupContent.Items.Clear();
        DdlGroupContent.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupContent.Items.Add(new ListItem(DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupContent.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.ContentKeyword.CapNhatNhomBaiVietMoi;
        ...
        {
            LtInsertUpdate.Text = Developer.ContentKeyword.TaoNhomBaiVietMoi;
            btn_insert_update.Text = "Đồng ý";
        }
        #endregion
    }
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupContent.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        }
        #endregion
        #region Update
        else
        {
            ...
            if (DdlGroupContent.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupContent.SelectedValue);
        }
        #endregion
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupContent.SelectedValue;
        txtDesc.Text = "";
