...
using Link = TatThanhJsc.CustomerReviewsModul.Link;

public partial class cms_admin_Moduls_CustomerReviews_Cate_ControlCate : System.Web.UI.UserControl
{
    ...
    {        
        return Link.LnkMnCustomerReviewsCateCreate();
    }

    private string LinkUpdateCate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(app, TypePage.UpdateCate, igid);        
        else
            return LinkAdmin.GoAdminCategory(app, TypePage.UpdateCate, igid, igparentid);        
    }
    #endregion
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_CustomerReviews('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
            s += "<a title='Click để xem và thêm mới " + CustomerReviewsKeyword.CustomerReviews2 + " cho danh mục này' href=\"" + UrlExtension.WebisteUrl + "admin.aspx?uc=" + app + "&igid=" + dt.Rows[i]["IGID"] + "&suc=" + TypePage.Item + "\"><span class='iconItem plus'><!----></span></a>";
    ...
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_CustomerReviews('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
