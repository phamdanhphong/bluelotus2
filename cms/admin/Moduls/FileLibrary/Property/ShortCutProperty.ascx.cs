...
public partial class cms_admin_Moduls_FileLibrary_Property_ShortCutProperty : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string app = CodeApplications.FileLibraryProperty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (hd_insert_update.Equals(TypePage.CreateProperty))
            insert = true;
    ...
    void KhoiTaoXuLyAnh()
    {
        ...
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhFileLibrary_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhFileLibrary_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhFileLibrary_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhFileLibrary_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhFileLibrary_TrongSuot, language);
        #endregion
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.FileLibrary, TypePage.Property, DdlGroupFileLibrary.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupFileLibrary.Items.Clear();
        DdlGroupFileLibrary.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupFileLibrary.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupFileLibrary.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.FileLibraryKeyword.CapNhatThuocTinh;
        ...
        {
            LtInsertUpdate.Text = Developer.FileLibraryKeyword.TaoThuocTinhMoi;
            btn_insert_update.Text = "Đồng ý";
        }
        #endregion
    }
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupFileLibrary.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg,ddlType.SelectedValue, "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        }
        #endregion
        #region Update
        else
        {
            ...
            if (DdlGroupFileLibrary.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupFileLibrary.SelectedValue);
        }
        #endregion
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupFileLibrary.SelectedValue;
        txtDesc.Text = "";
