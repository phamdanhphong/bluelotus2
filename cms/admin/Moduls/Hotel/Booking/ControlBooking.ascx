<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ControlBooking.ascx.cs" Inherits="cms_admin_Moduls_Hotel_Booking_ControlBooking" %>
...
<div id="ControlItem">
    <div id="ControlComment">
        <div class="BgTabTool">
        <a href="javascript:DeleteListSubItems()" class="LinkDelete"><%=Developer.HotelKeyword.Xoa%></a>                   
        <div class="right">            
    ...
    <div class="BgTabTitle" align="center">
        ...
        <div class="split">|</div>                    
        <div class="cot5">
            <asp:LinkButton ID="lbtStatus" runat="server" onclick="lbtStatus_Click" CssClass="arrowSort" ToolTip="Click để sắp xếp danh sách theo trường này"><%=Developer.HotelKeyword.TrangThai %></asp:LinkButton>
        </div>                        
        <div class="split">|</div>                    
        <div class="fr pr5"><%=Developer.HotelKeyword.CongCu %></div>                    
        <div class="cb"><!----></div>
    </div>
                ...
                <div class="Item" id="Item-<%#Eval("ISID").ToString()%>">
                    <div class="bgItem">
                        ...
                        <div class="cot2" align="left" style="line-height:18px">
                            <b>Hotel: <%#GetItemNameById(Eval("IID").ToString()) %></b><br/>
                            <b>Mã đơn hàng</b>: <%#Eval("VSTITLE").ToString() %><br/>
                            <b>Gửi lúc</b>: <%#Eval("dscreatedate").ToString() %><br/>
                            <b>Tổng tiền dự tính</b>: <%#NumberExtension.FormatNumber(Eval(SubitemsColumns.fsPrice).ToString()) %>VND<br/>
                            <b>Phương thức thanh toán</b>: <%#TourExtension.LayPhuongThucThanhToan(Eval(SubitemsColumns.vsParam1).ToString()) %><br/>
                            <b>Trạng thái thanh toán</b>: <%#TourExtension.LayTrangThaiThanhToan(Eval(SubitemsColumns.vsParam2).ToString()) %><br/>
                            <b>Thông tin khách hàng</b>
                            <ul>
                                <%#LayThongTinKhachHang(Eval("vscontent").ToString()) %>
                            </ul>
                            <b>Thông tin đặt phòng</b>
                            <ul>
                                <%#LayThongTinHotel(Eval("vsParams").ToString()) %>
                            </ul>
                            <div>
                                <%#LayThongTinHotel_BangDanhSachPhong(Eval("vsParams").ToString()) %>
                            </div>
                        </div>                            
                        <div class="split">|</div>                            
                        <div class="cot3"><%#TatThanhJsc.Extension.TimeExtension.FormatTime(Eval("DSCREATEDATE"),"dd/MM/yyyy - HH:mm")%></div>
                        <div class="split">|</div>                            
                        <div class="cot5" align="center">
                            <a onclick="return confirm('Bạn có chắc chắn muốn đổi trạng thái đơn hàng này (x: chưa xử lý xong; v: đã xử lý xong)?')" id="nc<%#Eval("ISID").ToString()%>" href="javascript:UpdateEnableSubItem(<%#Eval("ISID").ToString()%>)" class="EnableIcon<%#Eval("ISENABLE").ToString()%>">&nbsp;</a>
                        </div>
                        <div class="split">|</div>                        
