...
public partial class cms_admin_Moduls_Hotel_Facility_ControlFacility : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.HotelFacility;    
    ...
    {        
        return Link.LnkMnHotelFacilityCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Hotel, TypePage.UpdateFacility, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Hotel, TypePage.UpdateFacility, igid, igparentid);        
    }
    #endregion
