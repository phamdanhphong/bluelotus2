...
using Link = TatThanhJsc.HotelModul.Link;

public partial class cms_admin_Moduls_Hotel_Cate_ControlCate : System.Web.UI.UserControl
{
    ...
    {        
        return Link.LnkMnHotelCateCreate();
    }
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_Hotel('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
            s += "<a title='Click để xem và thêm mới " + HotelKeyword.Hotel2 + " cho danh mục này' href=\"" + UrlExtension.WebisteUrl + "admin.aspx?uc=" + app + "&igid=" + dt.Rows[i]["IGID"] + "&suc=" + TypePage.Item + "\"><span class='iconItem plus'><!----></span></a>";
