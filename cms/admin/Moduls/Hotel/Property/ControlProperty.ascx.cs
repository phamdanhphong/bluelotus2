...
public partial class cms_admin_Moduls_Hotel_Property_ControlProperty : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.HotelProperty;    
    ...
    {        
        return Link.LnkMnHotelPropertyCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Hotel, TypePage.UpdateProperty, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Hotel, TypePage.UpdateProperty, igid, igparentid);        
    }
    #endregion
