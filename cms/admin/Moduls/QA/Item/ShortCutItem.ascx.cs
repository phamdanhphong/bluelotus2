...
public partial class cms_admin_Moduls_QA_Item_ShortCutItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.QA;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string pic = FolderPic.QA;
    private string propertyModul = CodeApplications.QAProperty;
    ...
    {       
        //Ẩn hiện chức năng Thuộc tính hỏi đáp
        if (QAConfig.KeyHienThiQuanLyThuocTinhQA) pnThuocTinhQA.Visible = true;
        else pnThuocTinhQA.Visible = false;      
    }
    #endregion
    ...
    void KhoiTaoXuLyAnh()
    {
        ...
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhQA_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhQA_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhQA_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhQA_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhQA_TrongSuot, language);
        #endregion
    ...
    private string LinkRedrect()
    {
        ...
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.QA + "&igid=" + ddl_group_product.SelectedValue + "&suc="+TypePage.Item+"&ni=" + ni + "&p=" + p;
        }
        else
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.QA + "&igid=" + ddl_group_product.SelectedValue + "&suc=" + TypePage.Item;
        }
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            tbNguoiTraLoi.Text = dt.Rows[0][ItemsColumns.ViurlColumn].ToString();
    ...
    {
        string contentDetail = ContentExtendtions.ProcessStringContent(txt_content.Text, hdOldQA.Value, pic);
        #region Image
        string vimg = "";
        string vimg_thumb = "";
        ...
        string viparam = StringExtension.GhepChuoi("", tbEmail.Text, tbDienThoai.Text, tbDiaChi.Text, tbCongTy.Text);

        #region Insert
        if (insert)
        {
            GroupsItems.InsertItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, tbNguoiTraLoi.Text, tbNguoiGui.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", viparam, "0", "0", "", "", timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, status);
        ...
        else
        {
            ...
            GroupsItems.UpdateItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, tbNguoiTraLoi.Text, tbNguoiGui.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", viparam, "0", "0", "", HdIitotalview.Value, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text, status, iid);
