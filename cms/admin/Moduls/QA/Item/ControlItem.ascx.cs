...
public partial class cms_admin_Moduls_QA_Item_ControlItem : System.Web.UI.UserControl
{
    protected string app = CodeApplications.QA;
    protected string pic = FolderPic.QA;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string sortCookiesName = SortKey.SortQAItems;
    private string p = "1";
    ...
    protected string LinkUpdate(string iid)
    {
        ...
        {
            return LinkAdmin.GoAdminItem(CodeApplications.QA, TypePage.UpdateItem, iid, NumberShowItem, p);
        }
        else
        {
            return LinkAdmin.GoAdminItem(CodeApplications.QA, TypePage.UpdateItem, iid);
        }
    }
    ...
    protected string LinkCreate()
    {
        ...
        return LinkAdmin.GoAdminCategory(CodeApplications.QA, TypePage.CreateItem, igidUpdate);
    }
    ...
    void GetQA(string order)
    {
        ...
        LtPagging.Text = PagingExtension.SpilitPages(Convert.ToInt32(dt.Rows[0]["TotalRows"]),
                                                      ...
                                                      LinkAdmin.UrlAdmin(CodeApplications.QA, TypePage.Item,
    ...
    void GetGroupsInDdl()
    {
        ...
        ddlCateSearch.Items.Add(new ListItem(Developer.QAKeyword.TatCaDanhMuc, ""));
    ...
    {
        string key = tbKeySearch.Text + "&name=" + tbTitleSearch.Text;
        Response.Redirect(LinkAdmin.GoAdminCategory(CodeApplications.QA, TypePage.Item, ddlCateSearch.SelectedValue,
