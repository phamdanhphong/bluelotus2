...
public partial class cms_admin_Moduls_New_Item_ShortCutItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.News;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string pic = FolderPic.News;
    private string propertyModul = CodeApplications.NewsProperty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            if (Request.QueryString["copy"] == "1") copy = true;
        }

        Index1.btnHandler += new cms_api_New_Item_Index.OnButtonClick(WebUserControl1_btnHandler);
    ...
    {       
        //Ẩn hiện chức năng Thuộc tính tin tức
        if (NewConfig.KeyHienThiQuanLyThuocTinhTinTuc) pnThuocTinhTinTuc.Visible = true;
        else pnThuocTinhTinTuc.Visible = false;      
    }
    #endregion
    ...
    void KhoiTaoXuLyAnh()
    {
        ...
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhNews_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhNews_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhNews_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhNews_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhNews_TrongSuot, language);
        #endregion
    ...
    private string LinkRedrect()
    {
        ...
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.News + "&igid=" + ddl_group_product.SelectedValue + "&suc="+TypePage.Item+"&ni=" + ni + "&p=" + p;
        }
        else
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.News + "&igid=" + ddl_group_product.SelectedValue + "&suc=" + TypePage.Item;
        }
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            if (!copy)
               ...
               condition= DataExtension.AndConditon(ItemsTSql.GetItemsByIid(iid), ItemsTSql.GetItemsByIienable("1"));
            ...
            hdOldNew.Value = dt.Rows[0]["VICONTENT"].ToString();
            txtAuthor.Text= dt.Rows[0][ItemsColumns.ViauthorColumn].ToString();
            #region SEO
            ...
            if (!copy)
            {
                ...
                {
                    ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic,
                        dt.Rows[0]["VIIMAGE"].ToString(), "", "imgItem", false, false, "", false);
            ...
            #region ThuocTinhTinTuc-Chi thực hiện khi chức năng Quản lý thuộc tính được hiển thị
            if (NewConfig.KeyHienThiQuanLyThuocTinhTinTuc)
            {
                string properties = "";
                condition = TatThanhJsc.Extension.DataExtension.AndConditon(
    ...
    //{
    //    string contentDetail = ContentExtendtions.ProcessStringContent(txt_content.Text, hdOldNew.Value, pic);
    //    #region Image
    //    string vimg = "";
    //    string vimg_thumb = "";
    ...
    //    if (insert)
    //    {
    //        GroupsItems.InsertItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, "", txtAuthor.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", "", "0", "0", "", "", timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), "", ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), "", status);
    ...
    //            TatThanhJsc.Extension.ImagesExtension.DeleteImageWhenDeleteItem(pic, hd_img.Value);
    //        }
    //        GroupsItems.UpdateItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text, contentDetail, vimg, "", txtAuthor.Text, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", "", "0", "0", "", HdIitotalview.Value, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), "", ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), "", status, iid);
