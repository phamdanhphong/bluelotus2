...
public partial class cms_admin_Moduls_New_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Index1.btnHandler += new cms_api_New_Cate_Index.OnButtonClick(WebUserControl1_btnHandler);
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.News, TypePage.Cate, DdlGroupNew.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupNew.Items.Clear();
        DdlGroupNew.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupNew.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupNew.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_New", false, false, "", false);
            ...
            txt_content.Text = dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.VgcontentColumn].ToString();
    ...
    //    if (insert)
    //    {
    //        Groups.InsertGroups(language, app, DdlGroupNew.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
    ...
    //        if (DdlGroupNew.SelectedValue != hd_parent)
    //            Groups.UpdateParentOfGroups(igid, DdlGroupNew.SelectedValue);
    ...
    protected void WebUserControl1_btnHandler(string strValue)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupNew.SelectedValue, txt_title_modul.Text, txtDesc.Text, contentDetail, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupNew.SelectedValue;
        txtDesc.Text = "";
