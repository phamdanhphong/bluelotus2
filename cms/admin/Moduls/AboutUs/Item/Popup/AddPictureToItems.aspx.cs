...
public partial class cms_admin_ModulMain_Hotel_PopUp_Items_AddPictureToItems : System.Web.UI.Page
{
    ...
    string orderby = "";
    string condition = "";
    string app = CodeApplications.AboutUs +"image";
    string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    protected string iid = "";
    string pathFolderPic = FolderPic.AboutUs;
    ...
    {
        #region Đóng dấu ảnh
        if (SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs, language) == "1")
            cbDongDauAnh.Checked = true;
        else
            cbDongDauAnh.Checked = false;
        #region Ảnh làm dấu
        hdLogoImage.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_AnhDau, language);
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhAboutUs_TrongSuot, language);
        #endregion

        #region Hạn chế kích thước ảnh đại diện
        if (SettingsExtension.GetSettingKey(SettingKey.HanCheKichThuocAnhAboutUs, language) == "1")
            cbHanCheKichThuoc.Checked = true;
        else
            cbHanCheKichThuoc.Checked = false;

        tbHanCheW.Text = SettingsExtension.GetSettingKey(SettingKey.HanCheKichThuocAnhAboutUs_MaxWidth, language);
        tbHanCheH.Text = SettingsExtension.GetSettingKey(SettingKey.HanCheKichThuocAnhAboutUs_MaxHeight, language);
        #endregion

        #region Tạo ảnh nhỏ cho ảnh đại diện
        if (SettingsExtension.GetSettingKey(SettingKey.TaoAnhNhoChoAnhAboutUs, language) == "1")
            cbTaoAnhNho.Checked = true;
        else
            cbTaoAnhNho.Checked = false;

        tbAnhNhoW.Text = SettingsExtension.GetSettingKey(SettingKey.TaoAnhNhoChoAnhAboutUs_MaxWidth, language);
        tbAnhNhoH.Text = SettingsExtension.GetSettingKey(SettingKey.TaoAnhNhoChoAnhAboutUs_MaxHeight, language);
        #endregion
    }
    ...
    void GetItemsInfo()
    {
        ...
        ltrName.Text = @"
<div class='fwb'>
    Tên hình ảnh: " + dt.Rows[0][ItemsColumns.VititleColumn].ToString() + @"
</div>
<div class='cbh20'><!----></div>
";
        #endregion
    ...
    protected void btCancel_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect(UrlExtension.WebisteUrl + "cms/admin/Moduls/AboutUs/Item/PopUp/AddPictureToItems.aspx?iid=" + iid);
    }
    void GetList()
    {
    ...
    protected void rptList_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        switch (c)
        {
            ...
            case "edit":
                //OpenUpdatePanel(p);
                Response.Redirect(UrlExtension.WebisteUrl + "cms/admin/Moduls/AboutUs/Item/PopUp/AddPictureToItems.aspx?iid=" + iid + "&isid=" + p);
                break;
            #endregion
        }
    }
