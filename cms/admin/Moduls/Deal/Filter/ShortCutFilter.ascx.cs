...
public partial class cms_admin_Moduls_Deal_Filter_ShortCutFilter : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string app = CodeApplications.DealFilterProperties;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (hd_insert_update.Equals(TypePage.CreateFilter))
            insert = true;
    ...
    void GetListFilterType()
    {
        ...
        ddlFilterType.Items.Add(new ListItem("Chọn loại", ""));
        for (int i = 0; i < listFilterType.Values.Length; i++)
        {
            ddlFilterType.Items.Add(new ListItem(listFilterType.Text[i], listFilterType.Values[i]));
        }
    }

    private string LinkRedrect()
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Deal, TypePage.Filter, DdlGroupDeal.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupDeal.Items.Clear();
        DdlGroupDeal.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupDeal.Items.Add(new ListItem(DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupDeal.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.DealKeyword.CapNhatThuocTinh;
            ...
            ddlSubFilterMode.SelectedValue = dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.VgparamsColumn].ToString();
            ddlFilterType.SelectedValue = dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.IgtotalitemsColumn].ToString();
        }
        #endregion
        #region  insert
        else
        {
            LtInsertUpdate.Text = Developer.DealKeyword.TaoThuocTinhLocMoi;
            btn_insert_update.Text = "Đồng ý";
        }
        #endregion
    }
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        #endregion
        string vgParams = ddlSubFilterMode.SelectedValue;
        string param2 = ddlFilterType.SelectedValue;//Loại thuộc tính lọc, lưu vào igtotalitems
        #region Insert
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupDeal.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, vgParams, param2, txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        }
        #endregion
        #region Update
        else
        {
            ...
            Groups.UpdateGroups(language, app, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, vgParams, param2, txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status, igid);
            if (DdlGroupDeal.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupDeal.SelectedValue);
        }
        #endregion
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupDeal.SelectedValue;
        txtDesc.Text = "";
