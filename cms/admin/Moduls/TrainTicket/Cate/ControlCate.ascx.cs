...
public partial class cms_admin_Moduls_TrainTicket_Cate_ControlCate : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string Modul = CodeApplications.TrainTicket;    
    ...
    {        
        return Link.LnkMnTrainTicketCateCreate();
    }
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                s += "<div class=\"splitc\">|</div>";
                s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_TrainTicket('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
                s += "<div class=\"splitc\">|</div>";
                ...
                {
                    s += "<a title=\"Click để thêm các thuộc tính lọc cho danh mục này\" href=\"javascript:NewWindow_('cms/admin/Moduls/TrainTicket/Cate/PopUp/PopupAddFilterPropertiesToCategory.aspx?igid=" + dt.Rows[i]["IGID"].ToString() + "','ImageList','900','500','no','no')\"><span class='iconLocAdd'><!----></span></a>";
                    s += "&nbsp;&nbsp;&nbsp;";
                }
    ...
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_TrainTicket('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
