...
public partial class cms_admin_Moduls_Tag_Cate_ControlCate : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string Modul = CodeApplications.Tag;    
    ...
    {        
        return Link.LnkMnTagCreate();
    }
    
    private string LinkUpdateCate(string igid,string igparentid)
    {
        return LinkAdmin.GoAdminOther(CodeApplications.other, CodeApplications.Tag, TypePage.update, igid, "", "", "", "");
    }
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                s += "<div class=\"cot3\" align=\"center\">" + TagChoModul(dt.Rows[i][GroupsColumns.VgparamsColumn].ToString()) + "</div>";
                s += "<div class=\"splitc\">|</div>";

                s += "<div class=\"cot3\" align=\"center\">" + CountTagInGroups(dt.Rows[i]["IGID"].ToString()) + "</div>";
                s += "<div class=\"splitc\">|</div>";
                s += "<div class=\"cot4\" align=\"center\">" + GroupsExtension.CountChildCategory(dt.Rows[i]["IGID"].ToString(), "") + "</div>";
                s += "<div class=\"splitc\">|</div>";
                s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_Tag('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
                s += "<div class=\"splitc\">|</div>";
                ...
                s += "<div class=\"splitc\">|</div>";
                s += "<div class=\"cot7\">";
                s += "<a title='Click để đưa các bài viết vào tag này' href=\"javascript:void(0)\" onclick=\"NewWindow_('cms/admin/TempControls/PopUp/Cate/TagsInCate.aspx?igid=" + dt.Rows[i]["igid"].ToString() + "&Modul=" + dt.Rows[i]["vgparams"].ToString() + "','ImageList','950','500','yes','yes')\"><span class=\"ImageFolderAddRecord\"><!----></span></a>";
                s += "&nbsp;&nbsp;&nbsp;";
    ...
    private string TagChoModul(string p)
    {
        for (int i = 0; i < tagcfg.Values.Length; i++)
        {
            if (p == tagcfg.Values[i])
                return tagcfg.Text[i];
        }
        return "Tất cả";
    }
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderCate_Tag('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
