...
public partial class cms_admin_Moduls_Other_Vote_Cate_ShortCut : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string app = CodeApplications.Vote;
    private string pic = FolderPic.Vote;
    ...
    {
        return LinkAdmin.GoAdminOther(CodeApplications.other, CodeApplications.Vote, TypePage.Cate,
    ...
    void GetGroupsInDdl()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                DdlGroupSupportOnline.Items.Add(new ListItem(DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupSupportOnline.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", "", "", "", "", "", "", "", "", "", "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        }
        #endregion
        #region Update
        else
        {
            Groups.UpdateGroups(language, app, txt_title_modul.Text, txtDesc.Text, "", "", "", "", "", "", "", "", "", "", "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status, igid);
