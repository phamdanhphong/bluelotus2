...
public partial class cms_admin_Moduls_Video_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Video, TypePage.Cate, DdlGroupVideo.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupVideo.Items.Clear();
        DdlGroupVideo.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupVideo.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupVideo.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.VideoKeyword.CapNhatDanhMuc;
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_Video", false, false, "", false);
        ...
        {
            LtInsertUpdate.Text = Developer.VideoKeyword.TaoDanhMuc;
            btn_insert_update.Text = "Đồng ý";
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupVideo.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        ...
        else
        {
            ...
            if (DdlGroupVideo.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupVideo.SelectedValue);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupVideo.SelectedValue;
        txtDesc.Text = "";
