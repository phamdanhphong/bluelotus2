...
public partial class cms_admin_Moduls_Video_Item_ControlItem : System.Web.UI.UserControl
{
    protected string app = CodeApplications.Video;
    protected string pic = FolderPic.Video;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string sortCookiesName = SortKey.SortVideoItems;
    private string p = "1";
    ...
    protected string LinkUpdate(string iid)
    {
        ...
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Video, TypePage.UpdateItem, iid, NumberShowItem, p);
        }
        else
        {
            return LinkAdmin.GoAdminItem(CodeApplications.Video, TypePage.UpdateItem, iid);
        }
    }
    ...
    protected string LinkCreate()
    {
        ...
        return LinkAdmin.GoAdminCategory(CodeApplications.Video, TypePage.CreateItem, igidUpdate);
    }

    void GetVideo(string order)
    {
        if (!igid.Equals(""))
        {
        ...
        LtPagging.Text = PagingExtension.SpilitPages(Convert.ToInt32(dt.Rows[0]["TotalRows"]),
                                                      ...
                                                      LinkAdmin.UrlAdmin(CodeApplications.Video, TypePage.Item,
    ...
    void GetGroupsInDdl()
    {
        ...
        ddlCateSearch.Items.Add(new ListItem(Developer.VideoKeyword.TatCaDanhMuc, ""));
    ...
    {
        string key = tbKeySearch.Text + "&name=" + tbTitleSearch.Text;
        Response.Redirect(LinkAdmin.GoAdminCategory(CodeApplications.Video, TypePage.Item, ddlCateSearch.SelectedValue,
