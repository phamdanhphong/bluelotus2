...
public partial class cms_admin_Moduls_Tour_Vehicle_ControlVehicle : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.TourVehicle;    
    ...
    {        
        return Link.LnkMnTourVehicleCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateVehicle, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateVehicle, igid, igparentid);        
    }
    #endregion
