...
public partial class cms_admin_Moduls_Tour_Property_ControlProperty : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.TourProperty;    
    ...
    {        
        return Link.LnkMnTourPropertyCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateProperty, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateProperty, igid, igparentid);        
    }
    #endregion
