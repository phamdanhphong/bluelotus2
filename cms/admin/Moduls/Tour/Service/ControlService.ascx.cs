...
public partial class cms_admin_Moduls_Tour_Service_ControlService : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.TourService;    
    ...
    {        
        return Link.LnkMnTourServiceCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateService, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Tour, TypePage.UpdateService, igid, igparentid);        
    }
    #endregion
