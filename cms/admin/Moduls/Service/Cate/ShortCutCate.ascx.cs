...
public partial class cms_admin_Moduls_Service_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Service, TypePage.Cate, DdlGroupService.SelectedValue);
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.ServiceKeyword.CapNhatDanhMuc;
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_Service", false, false, "", false);
            ...
            tbLuotXem.Text= dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.IgTotalItems].ToString();
            ...
            hdOldService.Value = dt.Rows[0]["VGCONTENT"].ToString();
        }
        #endregion
        #region  insert
        else
        {
            LtInsertUpdate.Text = Developer.ServiceKeyword.TaoDanhMuc;
            btn_insert_update.Text = "Đồng ý";
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupService.SelectedValue, txt_title_modul.Text, txtDesc.Text, contentDetail, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", tbLuotXem.Text, txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        ...
        else
        {
            ...
            if (DdlGroupService.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupService.SelectedValue);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupService.SelectedValue;
        txtDesc.Text = "";
