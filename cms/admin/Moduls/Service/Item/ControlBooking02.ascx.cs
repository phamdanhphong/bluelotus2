...
public partial class cms_admin_Moduls_service_Booking_ControlBooking02 : System.Web.UI.UserControl
{
    protected string app = CodeApplications.Service;
    protected string appCate = CodeApplications.Service;
    ...
    private string ExtractSubInfo(DataTable dt, string parentName)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            if (dt.Rows[i]["type"].ToString() == "text")            
                ...
                if (dt.Rows[i]["type"].ToString() == "radio")
                s += dt.Rows[i]["name"].ToString() != parentName
                ? "<li>" + dt.Rows[i]["name"] + ": " + dt.Rows[i]["data"] + "</li>"
                : "<li>" + dt.Rows[i]["data"].ToString().Replace("\n", "<br/>") + "</li>";
    ...
    void GetParentCate()
    {
        ...
        ddlCateSearch.Items.Add(new ListItem("Chọn bài viết cần xem bình luận", ""));
        for (int i = 0; i < dt.Rows.Count; i++)
        {
