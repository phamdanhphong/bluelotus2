...
public partial class cms_admin_Moduls_Service_Item_ShortCutItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.Service;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string pic = FolderPic.Service;
    private string propertyModul = CodeApplications.ServiceProperty;
    ...
    {       
        //Ẩn hiện chức năng Thuộc tính dịch vụ
        if (ServiceConfig.KeyHienThiQuanLyThuocTinhDichVu) pnThuocTinhDichVu.Visible = true;
        else pnThuocTinhDichVu.Visible = false;      
    }
    #endregion
    ...
    private string LinkRedrect()
    {
        ...
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Service + "&igid=" + ddl_group_product.SelectedValue + "&suc="+TypePage.Item+"&ni=" + ni + "&p=" + p;
        }
        else
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Service + "&igid=" + ddl_group_product.SelectedValue + "&suc=" + TypePage.Item;
        }
    }
