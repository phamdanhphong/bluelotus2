...
public partial class cms_admin_Moduls_PhotoAlbum_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.PhotoAlbum, TypePage.Cate, DdlGroupPhotoAlbum.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupPhotoAlbum.Items.Clear();
        DdlGroupPhotoAlbum.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupPhotoAlbum.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupPhotoAlbum.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.PhotoAlbumKeyword.CapNhatDanhMuc;
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_PhotoAlbum", false, false, "", false);
        ...
        {
            LtInsertUpdate.Text = Developer.PhotoAlbumKeyword.TaoDanhMuc;
            btn_insert_update.Text = "Đồng ý";
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (DdlGroupPhotoAlbum.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupPhotoAlbum.SelectedValue);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupPhotoAlbum.SelectedValue;
        txtDesc.Text = "";
