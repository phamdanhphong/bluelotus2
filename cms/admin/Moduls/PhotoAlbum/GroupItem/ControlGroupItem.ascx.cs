...
public partial class cms_admin_Moduls_PhotoAlbum_GroupItem_ControlGroupItem : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.PhotoAlbumGroupItem;
    string appAddItem = CodeApplications.PhotoAlbum;
    private string typeModul = CodeApplications.PhotoAlbum;
    ...
    {
        return Link.LnkMnPhotoAlbumGroupItemCreate();
    }
    ...
    {
        string s = "_";
        PhotoAlbumPosition listModul = new PhotoAlbumPosition();
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderGroupItem_PhotoAlbum('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
    ...
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"cot4\" align=\"center\">" + SetPosition(dt.Rows[i]["VGPARAMS"].ToString()) + "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderGroupItem_PhotoAlbum('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
