...
public partial class cms_admin_Moduls_Blog_Item_ShortCutItem : System.Web.UI.UserControl
{
    private string app = CodeApplications.Blog;
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();
    private string pic = FolderPic.Blog;
    private string propertyModul = CodeApplications.BlogProperty;
    ...
    {       
        //Ẩn hiện chức năng Thuộc tính blog
        if (BlogConfig.KeyHienThiQuanLyThuocTinhBlog) pnThuocTinhBlog.Visible = true;
        else pnThuocTinhBlog.Visible = false;      
    }
    #endregion
    ...
    void KhoiTaoXuLyAnh()
    {
        ...
        #endregion
        hdViTriDongDau.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhBlog_ViTri, language);
        hdLeX.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhBlog_LeNgang, language);
        hdLeY.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhBlog_LeDoc, language);
        hdTyLe.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhBlog_TyLe, language);
        hdTrongSuot.Value = SettingsExtension.GetSettingKey(SettingKey.DongDauAnhBlog_TrongSuot, language);
        #endregion
    ...
    private string LinkRedrect()
    {
        ...
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Blog + "&igid=" + ddl_group_product.SelectedValue + "&suc="+TypePage.Item+"&ni=" + ni + "&p=" + p;
        }
        else
        {
            return UrlExtension.WebisteUrl + "admin.aspx?uc=" + CodeApplications.Blog + "&igid=" + ddl_group_product.SelectedValue + "&suc=" + TypePage.Item;
        }
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            hdOldBlog.Value = dt.Rows[0]["VICONTENT"].ToString();
            #region SEO
            ...
            tbAuthor.Text = dt.Rows[0][ItemsColumns.ViauthorColumn].ToString();

            #region ThuocTinhBlog-Chi thực hiện khi chức năng Quản lý thuộc tính được hiển thị
            if (BlogConfig.KeyHienThiQuanLyThuocTinhBlog)
            {
                string properties = "";
                condition = TatThanhJsc.Extension.DataExtension.AndConditon(
    ...
    {
        string contentDetail = ContentExtendtions.ProcessStringContent(txt_content.Text, hdOldBlog.Value, pic);
        #region Image
        string vimg = "";
        string vimg_thumb = "";
        ...
        if (insert)
        {
            GroupsItems.InsertItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text,
                contentDetail, vimg, "", tbAuthor.Text, textTagTitle.Text, textLinkRewrite.Text,
                StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "",
                "", "", "0", "0", "", "", timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(), tbOrder.Text,
                ddl_group_product.SelectedValue, timeCreateDate, DateTime.Now.ToString(), DateTime.Now.ToString(),
                tbOrder.Text, status);
        ...
        else
        {
            ...
            GroupsItems.UpdateItemsGroupsItems(language, app, tbKey.Text, txt_title.Text, txt_description.Text,
                contentDetail, vimg, "", tbAuthor.Text, textTagTitle.Text, textLinkRewrite.Text,
                StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "",
                "", "", "0", "0", "", HdIitotalview.Value, timeCreateDate, DateTime.Now.ToString(),
