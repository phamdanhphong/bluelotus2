...
public partial class cms_admin_Moduls_FileLibrary2_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.FileLibrary2, TypePage.Cate, DdlGroupFileLibrary2.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupFileLibrary2.Items.Clear();
        DdlGroupFileLibrary2.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupFileLibrary2.Items.Add(new ListItem(TatThanhJsc.Extension.DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupFileLibrary2.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            LtInsertUpdate.Text = Developer.FileLibrary2Keyword.CapNhatDanhMuc;
            ...
            ltimg.Text = TatThanhJsc.Extension.ImagesExtension.GetImage(pic, dt.Rows[0]["VGIMAGE"].ToString(), "", "adm_img_FileLibrary2", false, false, "", false);
        ...
        {
            LtInsertUpdate.Text = Developer.FileLibrary2Keyword.TaoDanhMuc;
            btn_insert_update.Text = "Đồng ý";
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupFileLibrary2.SelectedValue, txt_title_modul.Text, txtDesc.Text, "", textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, "", "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        ...
        else
        {
            ...
            if (DdlGroupFileLibrary2.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupFileLibrary2.SelectedValue);
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupFileLibrary2.SelectedValue;
        txtDesc.Text = "";
