...
public partial class cms_admin_Moduls_FileLibrary2_Property_RecycleProperty : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string Modul = CodeApplications.FileLibrary2Property;
    protected string pic = FolderPic.FileLibrary2;
    ...
    void GetAllModul()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                str += "<div class=\"splitc\">|</div>";
                str += "<div class=\"cot7\">";
                str += "<a href=\"javascript:RestoreGroup('" + dt.Rows[i]["IGID"].ToString() + "','" + StatusGroupParent(dt.Rows[i]["IGPARENTID"].ToString()) + "','" + GetSubProperty(dt.Rows[i]["IGID"].ToString()) + "','" + dt.Rows[i]["VGNAME"].ToString() + "')\"><span class='iconRestore'><!----></span></a>";
                str += "&nbsp;&nbsp;&nbsp;";
                str += "<a href=\"javascript:DeleteRecGroup('" + dt.Rows[i]["IGID"].ToString() + "','" + GetSubProperty(dt.Rows[i]["IGID"].ToString()) + "','" + dt.Rows[i]["VGNAME"].ToString() + "','" + pic + "')\"><span class='iconDelete'><!----></span></a>";
                str += "</div>";
                str += "<div class=\"cbh0\"><!----></div>";
                str += "</div>";
                str += "</div>";
    ...
    string GetSubProperty(string igparentid)
    {
        top = "";
        fields = " * ";
        condition = DataExtension.AndConditon(GroupsTSql.GetGroupsByVgapp(Modul),
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (!CountProperty(dt.Rows[i]["IGID"].ToString()).Equals("0"))
                {
                    str += GetSubProperty(dt.Rows[i]["IGID"].ToString()) + ",";
                }
            }
    ...
    string CountProperty(string igid)
    {
        top = "";
        fields = " * ";
        condition = DataExtension.AndConditon(GroupsTSql.GetGroupsByVgapp(Modul),
    ...
    string GetRoadGroup(string igid, string vgPropertyCurrent)
    {
        string str = "";
        top = "1";
        fields = " * ";
