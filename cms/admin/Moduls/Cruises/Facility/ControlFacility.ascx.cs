...
public partial class cms_admin_Moduls_Cruises_Facility_ControlFacility : System.Web.UI.UserControl
{
    private string lang = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string app = CodeApplications.CruisesFacility;    
    ...
    {        
        return Link.LnkMnCruisesFacilityCreate();
    }

    private string LinkUpdateCreate(string igid, string igparentid)
    {
        if (igparentid.Equals(""))
            return LinkAdmin.GoAdminCategory(CodeApplications.Cruises, TypePage.UpdateFacility, igid);        
        else
            return LinkAdmin.GoAdminCategory(CodeApplications.Cruises, TypePage.UpdateFacility, igid, igparentid);        
    }
    #endregion
