...
public partial class cms_admin_Moduls_ContactUs_Cate_ShortCutCate : System.Web.UI.UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubModul(CodeApplications.Contact, TypePage.Cate, DdlGroupContactUs.SelectedValue);
    }
    ...
    void GetGroupsInDdl()
    {
        ...
        DdlGroupContactUs.Items.Clear();
        DdlGroupContactUs.Items.Add(new ListItem("Danh mục gốc", "0"));
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DdlGroupContactUs.Items.Add(new ListItem(DropDownListExtension.FormatForDdl(dt.Rows[i]["IGLEVEL"].ToString()) + dt.Rows[i]["VGNAME"].ToString(), dt.Rows[i]["IGID"].ToString()));
            }
        }
        DdlGroupContactUs.SelectedValue = hd_parent;
    }
    ...
    void InitialControlsValue(bool insert)
    {
        ...
        if (!insert)
        {
            ...
            txtMaps.Text = dt.Rows[0][TatThanhJsc.Columns.GroupsColumns.VgParams].ToString();
            ...
            tbHotline.Text = StringExtension.LayChuoi(content, "", 8);
    ...
    protected void btn_insert_update_Click(object sender, EventArgs e)
    {
        ...
        string content = StringExtension.GhepChuoi("", tbDiaChi.Text, tbDienThoai.Text, tbFax.Text, tbEmail.Text, tbKinhDo.Text, tbViDo.Text,tbWebsite.Text, tbHotline.Text);        

        #region Insert
        if (insert)
        {
            Groups.InsertGroups(language, app, DdlGroupContactUs.SelectedValue, txt_title_modul.Text, txtDesc.Text, content, textTagTitle.Text, textLinkRewrite.Text, StringExtension.ReplateTitle(textLinkRewrite.Text), textTagKeyword.Text, textTagDescription.Text, "", "", "", vimg, txtMaps.Text, "", txt_ordermodul.Text, DateTime.Now.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), status);
        }
        #endregion
        #region Update
        else
        {
            ...
            if (DdlGroupContactUs.SelectedValue != hd_parent)
                Groups.UpdateParentOfGroups(igid, DdlGroupContactUs.SelectedValue);
        }
        #endregion
    ...
    void ResetControls()
    {
        ...
        ltimg.Text = "";
        hd_img.Value = "";
        hd_parent = DdlGroupContactUs.SelectedValue;
        txtDesc.Text = "";
