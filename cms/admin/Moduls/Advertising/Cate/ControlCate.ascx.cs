...
public partial class cms_admin_Moduls_Advertising_Cate_ControlCate : System.Web.UI.UserControl
{
    private string language = TatThanhJsc.LanguageModul.Cookie.GetLanguageValueAdmin();

    private string Modul = CodeApplications.Advertising;
    private string typePageCreate = TypePage.CreateCate;
    ...
    {
        return LinkAdmin.GoAdminSubModul(Modul, typePageCreate);        
    }
    ...
    private string LinkUpdateCate(string igid,string igparentid)
    {
        ...
        {
            return LinkAdmin.GoAdminCategory(Modul, typePageUpdate, igid);    
        }
        else
        {
            return LinkAdmin.GoAdminCategory(Modul, typePageUpdate, igid, igparentid);    
        }
    }
    ...
    void GetCate()
    {
        ...
        orderBy = "VGPARAMS, IGORDER, VGNAME";
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot3\" align=\"center\">" + GetPosition(dt.Rows[i]["VGPARAMS"].ToString()) + "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot4\" align=\"center\">" + GroupsExtension.CountItemInGroups(dt.Rows[i]["IGID"].ToString(), "") + "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderGroup_Advertising('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
    ...
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"cot4\" align=\"center\">" + GroupsExtension.CountItemInGroups(dt.Rows[i]["IGID"].ToString()) + "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox\" onchange=\"UpdateOrderGroup_Advertising('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7\">";
